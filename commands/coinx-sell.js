...
program
	...
	.option('-e, --exchange [name]', 'A specific exchange to sell to.')
	.parse(process.argv);
...
if (program.exchange) {
	...
	console.log(chalk.blue('Selling on ' + capitalize(program.exchange) + '...'));
} else {
	...
	console.log(chalk.blue('Selling...'));
}
...
if (program.usd) {
	Promise.all([
		...
		.then(results => {
			...
			let numCoinsToSell = (program.usd / (bestMarket.priceBTC * btcPrice)).toFixed(8);
			...
			let questions = [{
				type: 'confirm',
				name: 'proceed',
				message: 'Sell about ' + numCoinsToSell + ' worth of ' + symbol + '?'
			}];
			...
			inquirer
				...
				.then(results => {
					...
					console.log(chalk.green('Selling...'));
					return exchanges[bestMarket.exchange].sell(symbol, program.usd);
				})
				.then(result => {
					if (result.complete) {
						console.log(chalk.green('Order complete!'));
						console.log(chalk.green(capitalize(result.market) + ' order number ' + result.orderNumber));
						console.log(chalk.green('Sold ' + result.numCoinsBought + ' ' + symbol + ' at ' + result.rate + ' BTC per coin'));
